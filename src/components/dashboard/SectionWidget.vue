<template>
  <div class="widget p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg transition-all">
    ...
    <div class="grid grid-cols-4 gap-6">
      <div 
        ...
        :key="section.id" 
        class="widget p-6 bg-gray-200 dark:bg-gray-900 rounded-lg shadow-lg transition-all relative"
      >
        <div 
          :class="section.type === 'Inbound' ? 'bg-green-500' : 'bg-blue-500'"
  ...
  <div 
    ...
    class="absolute p-2 bg-gray-900 text-white text-sm rounded-lg shadow-lg transition-opacity duration-200"
  >
    <p><strong>ID:</strong> {{ tooltip.data.id }}</p>
    <p>
      <strong>Status: </strong> 
      <span :class="tooltip.data.occupied ? 'text-red-500' : 'text-green-500'">
...
const slotClasses = (occupied) => {
  return occupied ? 'bg-red-500 animate-pulse' : 'bg-green-500';
};
